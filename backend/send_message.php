...
use function PHPSTORM_META\type;
    ...
    if ($result -> num_rows === 0){
        ...
        $qry = "UPDATE chats 
            SET message = ?, sender = '$userId', reciever = '$recieverId' 
            WHERE (sender = ? AND reciever = ?) OR (sender = ? AND reciever = ?)
        ";
        $stmt = mysqli_prepare($con, $qry);
        mysqli_stmt_bind_param($stmt, "sssss", $message, $userId, $recieverId, $recieverId, $userId);
    ...
    if($result){
        ...
        $sql = "insert into notifications (user_id, other_user, note_message, note_type, status) 
        values ('$recieverId', '$userId', '$message', 'message', 'sent')";
        $note_res = mysqli_query($con, $sql);
